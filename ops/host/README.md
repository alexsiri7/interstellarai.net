...
and, since NodeSource moved to Node 22, restart the archon user services once as
...
| 6. NodeSource | `/etc/apt/sources.list.d/nodesource.sources` (backup kept alongside) | Rewrites `node_20.x` to `node_22.x` in the Suites/URIs (Node 20 is EOL), `apt-get update`, `apt-get install -y nodejs`. |
...
per running kernel ("reboot pending; auto-reboot at 04:30"), remembered in
...
`/var/run/reboot-required` it schedules the reboot for the next 04:30. Archon
